...
class BiasBoardWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # 先设置布局，这样后续方法可以使用 self.layout()
        self.setLayout(layout)
        
        # 根据板卡ID显示不同的通道控制
        if self.board_id == 0:  # 第一块板卡
            self.setup_board1_channels()
        elif self.board_id == 1:  # 第二块板卡
    ...
    def setup_board1_channels(self):
        ...
        for i in range(16):
            ...
            layout.addWidget(switch, i//4 + 1, (i%4)*2)
            ...
            layout.addWidget(value, i//4 + 1, (i%4)*2 + 1)
        ...
        sq_main_layout = QVBoxLayout()
        
        # 方波参数设置
        sq_param_layout = QGridLayout()
        sq_param_layout.addWidget(QLabel("方波幅值:"), 0, 0)
        ...
        sq_param_layout.addWidget(self.square_amplitude, 0, 1)
        
        sq_param_layout.addWidget(QLabel("方波频率:"), 0, 2)
        ...
        sq_param_layout.addWidget(self.square_frequency, 0, 3)
        
        sq_main_layout.addLayout(sq_param_layout)
        
        # 16列方波信号控制（四列布局）
        sq_channel_layout = QGridLayout()
        ...
        sq_channel_layout.addWidget(self.square_master_switch, 0, 0, 1, 12)
        ...
        # 四列布局：每列包含4个通道
        for i in range(16):
            col_group = i // 4  # 0, 1, 2, 3 (确定在哪一列组)
            row_in_group = i % 4  # 0, 1, 2, 3 (在该列组中的行位置)
            
            base_col = col_group * 3  # 每组占3列（开关标签 + 开关 + 输出值）
            base_row = row_in_group + 1  # 从第1行开始（第0行是总开关）
            
            # 列标签（开关）
            switch = QCheckBox(f"列{i+1}")
            self.square_switches.append(switch)
            sq_channel_layout.addWidget(switch, base_row, base_col)
            ...
            sq_channel_layout.addWidget(value, base_row, base_col + 1)
        
        sq_main_layout.addLayout(sq_channel_layout)
        self.square_wave_group.setLayout(sq_main_layout)
        self.layout().addWidget(self.square_wave_group)
        
        # 初始隐藏方波信号控制（仅在交流模式下显示）
        self.square_wave_group.hide()
    ...
    def setup_board2_channels(self):
        ...
        self.sa_ib_master_switch.stateChanged.connect(self.toggle_all_sa_ib)
        ...
        self.sa_phix_master_switch.stateChanged.connect(self.toggle_all_sa_phix)
        layout1.addWidget(self.sa_phix_master_switch, 0, 3, 1, 2)
        
        # 添加表头
        layout1.addWidget(QLabel("列"), 1, 0)
        layout1.addWidget(QLabel("SA Ib开关"), 1, 1)
        layout1.addWidget(QLabel("SA Ib输出值"), 1, 2)
        layout1.addWidget(QLabel("SA phix开关"), 1, 3)
        layout1.addWidget(QLabel("SA phix输出值"), 1, 4)
        
        self.sa_ib_switches = []
        self.sa_ib_values = []
        self.sa_phix_switches = []
        self.sa_phix_values = []
        
        for i in range(16):
            # 列标签
            layout1.addWidget(QLabel(f"列{i+1}"), i+2, 0)
            
            # SA Ib开关
            ib_switch = QCheckBox()
            self.sa_ib_switches.append(ib_switch)
            layout1.addWidget(ib_switch, i+2, 1)
        ...
        self.vb_master_switch.stateChanged.connect(self.toggle_all_vb)
        ...
        for i in range(16):
            # 列标签
            layout2.addWidget(QLabel(f"列{i+1}"), i+2, 0)
            
            # Vb开关
            switch = QCheckBox()
            self.vb_switches.append(switch)
            layout2.addWidget(switch, i+2, 1)
            ...
            layout2.addWidget(value, i+2, 2)
            
        group2.setLayout(layout2)
        self.layout().addWidget(group2)
    
    def toggle_all_sa_ib(self, state):
        """切换所有SA Ib通道"""
        checked = (state == 2)
        for switch in self.sa_ib_switches:
            switch.setChecked(checked)
    
    def toggle_all_sa_phix(self, state):
        """切换所有SA phix通道"""
        checked = (state == 2)
        for switch in self.sa_phix_switches:
            switch.setChecked(checked)
    
    def toggle_all_vb(self, state):
        """切换所有Vb通道"""
        checked = (state == 2)
        for switch in self.vb_switches:
            switch.setChecked(checked)
    ...
    def setup_board3_channels(self):
        ...
        self.is_i_master_switch.stateChanged.connect(self.toggle_all_is_i)
        ...
        self.is_phib_master_switch.stateChanged.connect(self.toggle_all_is_phib)
        layout.addWidget(self.is_phib_master_switch, 0, 3, 1, 2)
        
        # 添加表头
        layout.addWidget(QLabel("列"), 1, 0)
        layout.addWidget(QLabel("IS I开关"), 1, 1)
        layout.addWidget(QLabel("IS I输出值"), 1, 2)
        layout.addWidget(QLabel("IS phib开关"), 1, 3)
        layout.addWidget(QLabel("IS phib输出值"), 1, 4)
        
        self.is_i_switches = []
        self.is_i_values = []
        self.is_phib_switches = []
        self.is_phib_values = []
        ...
        for i in range(16):
            ...
            layout.addWidget(i_switch, i+2, 1)
            ...
            layout.addWidget(i_value, i+2, 2)
            
            # IS phib开关
            phib_switch = QCheckBox()
            self.is_phib_switches.append(phib_switch)
            layout.addWidget(phib_switch, i+2, 3)
            
            # IS phib输出值
            phib_value = QDoubleSpinBox()
            phib_value.setRange(-1000, 1000)
            phib_value.setValue(0)
            phib_value.setSuffix(" uA")
            self.is_phib_values.append(phib_value)
            layout.addWidget(phib_value, i+2, 4)
            
        group.setLayout(layout)
        self.layout().addWidget(group)
    
    def toggle_all_is_i(self, state):
        """切换所有IS I通道"""
        checked = (state == 2)
        for switch in self.is_i_switches:
            switch.setChecked(checked)
    
    def toggle_all_is_phib(self, state):
        """切换所有IS phib通道"""
        checked = (state == 2)
        for switch in self.is_phib_switches:
            switch.setChecked(checked)
    ...
    def get_parameters(self):
        ...
        params = {
            ...
            'waveform': self.waveform_type.currentText() if hasattr(self, 'waveform_type') else '',
            'frequency': self.frequency.value() if hasattr(self, 'frequency') else 0,
            'amplitude': self.amplitude.value() if hasattr(self, 'amplitude') else 0,
            'dc_value': self.dc_value.value() if hasattr(self, 'dc_value') else 0
        }
        
        # 如果是第一块板卡，添加方波信号参数
        if self.board_id == 0 and hasattr(self, 'square_amplitude'):
            params['square_wave'] = {
                'amplitude': self.square_amplitude.value(),
                'frequency': self.square_frequency.value(),
                'master_switch': self.square_master_switch.isChecked(),
                'channels': []
            }
            for i in range(16):
                params['square_wave']['channels'].append({
                    'column': i + 1,
                    'enabled': self.square_switches[i].isChecked(),
                    'output_value': self.square_values[i].value()
                })
