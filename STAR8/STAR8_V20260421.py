...
class StarCryoControlGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("STAR Cryoelectronics Control System")
    ...
    def create_serial_monitor(self):
        """创建串口数据监控窗口"""
        monitor_frame = ttk.LabelFrame(self.main_container, text="Operation Log & Serial Monitor", padding="10")
    ...
    def s_lock(self):
        ...
        for ch in self.get_selected_channels():
            ...
            command = f"SLOCK {ch}"
            self.log_operation("S-LOCK Mode", {"channel": ch, "command": command})
    ...
    def s_tune(self):
        ...
        for ch in self.get_selected_channels():
            ...
            command = f"STUNE {ch}"
            self.log_operation("S-TUNE Mode", {"channel": ch, "command": command})
    ...
    def a_lock(self):
        ...
        for ch in self.get_selected_channels():
            ...
            command = f"ALOCK {ch}"
            self.log_operation("A-LOCK Mode", {"channel": ch, "command": command})
    ...
    def a_tune(self):
        ...
        for ch in self.get_selected_channels():
            ...
            command = f"ATUNE {ch}"
            self.log_operation("A-TUNE Mode", {"channel": ch, "command": command})
