...
class StarCryoControlGUI:
    ...
    def create_main_layout(self):
        ...
        ttk.Label(squid_frame, text="S-BIAS (µA):").grid(row=0, column=0, sticky="w", pady=5)
        ...
        ttk.Combobox(feedback_frame, textvariable=self.feedback, 
                    values=["10kOhm","30kOhm", "100kOhm"], width=12).grid(row=1, column=1, pady=5)
    ...
    def set_parameter(self, param):
            ...
            if param == "offset":
                unit = "mV"
                v_out = physical_value * 0.001
                
                # Code = 2048 * (V_out / 5.0) + 2048
                dac_code_int = int(2048 * (v_out / 5.0) + 2048)
                
                log_v_info = f"{v_out:.4f}V (Bipolar)"
                
            else:
                unit = "µA"
                # 电压绝对值 = I * R = (val * 1e-6) * 102400 = val * 0.1024
                # 因为只能输出同一方向的电流，这里取绝对值防呆
            ...
            dac_12bit = f"{dac_code_int:012b}" # 转为 12 位纯 0/1 字符串

            # 3. DAC 
            for ch in self.get_selected_channels():
                
                # --- 生成后 4 位 DAC 通道地址 (A0 + DS2~DS0) ---
                # 奇数通道 A0=1，偶数 A0=0
                a0_bit = "1" if ch % 2 != 0 else "0"
                # 计算 DS 地址 (1/2通道=000, 3/4通道=001, 5/6=010...)
                ds_val = (ch - 1) // 2 
                ds_3bit = f"{ds_val:03b}"
                
                # 组合 DAC 通道地址
                dac_addr_4bit = a0_bit + ds_3bit
                
                # 16-bit 数据：[12位DAC电压值] + [4位DAC通道地址]
                data_16bit = dac_12bit + dac_addr_4bit
                
                #  命令类型 "10"(偏置相关)，命令对象暂定 "00" 
                cmd_obj = "00"  
                
                # 拼接
                bin_str, raw_bytes = self.build_fpga_command_binary("10", cmd_obj, ch, data_16bit)
                
                # 日志显示：MAC | 偏置对象通道 | 12位DAC | 4位DAC地址
